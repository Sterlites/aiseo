...
function analyzeMetaDescription($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  if (!metaDescription) {
    return {
      ...
      details: {
        ...
        impact: "negative",
        context:
          "A meta description is crucial for SEO and click-through rates.",
      },
    };
  }
  ...
  if (descLength < 120) {
    ...
    impact = "neutral";
    context =
      "Meta description exceeds 160 characters. Consider shortening it for optimal display in search results.";
  } else {
    impact = "positive";
  ...
  const keywordInDescription = keywords.some((keyword) =>
  ...
  if (!keywordInDescription) {
    score -= 10;
    context +=
      " Consider including a relevant keyword from the title in the meta description.";
  }
...
function analyzeHeadings($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  if (h2Count === 0) {
    score -= 10;
    context +=
      " No H2 headings found. Consider using H2 tags to structure your content.";
  } else {
  ...
  if (!keywordInH1 && h1Count > 0) {
    score -= 10;
    context +=
      " Consider including a relevant keyword from the title in the H1 tag.";
  }
...
function analyzeMobileFriendliness($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  if (!viewport) {
    ...
    impact = "negative";
    context =
      "No viewport meta tag found. This is crucial for mobile responsiveness.";
  } else if (
    ...
    impact = "negative";
    context =
      "Viewport meta tag is present but may not be optimally configured for mobile devices.";
  } else {
    impact = "positive";
...
function analyzeLinkingStructure($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  if (internalLinks === 0) {
    ...
    impact = "negative";
    context =
      "No internal links found. Consider adding links to other pages on your site.";
  ...
  if (externalLinks === 0) {
    score -= 10;
    context +=
      " No external links found. Consider adding links to authoritative sources.";
  } else {
...
function analyzeContentQuality($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  const bodyText = $("body")
    .clone()
    .children("script, style, noscript")
    .remove()
    .end()
    .text()
    .trim();
  ...
  if (wordCount < 300) {
    ...
    impact = "negative";
    context =
      "Content length is too short. Aim for at least 300 words of quality content.";
  } else if (wordCount < 600) {
    score -= 15;
    impact = "neutral";
    context =
      "Content length is moderate. Consider expanding to at least 600 words for more comprehensive coverage.";
  } else {
    impact = "positive";
  ...
  if (noscriptContent.length > 0) {
    ...
    context +=
      " Noscript content present, which is good for accessibility and SEO.";
  }
  ...
  const keywordDensity = allKeywords.reduce((density, keyword) => {
    const count = (
      combinedText.toLowerCase().match(new RegExp(`\\b${keyword}\\b`, "g")) ||
      []
    ).length;
  ...
  if (keywordDensity < 0.01) {
    score -= 10;
    context +=
      " Keyword density is low. Consider naturally incorporating more relevant keywords.";
  } else if (keywordDensity > 0.03) {
    score -= 5;
    context +=
      " Keyword density is high. Ensure the content reads naturally and isn't over-optimized.";
  } else {
  ...
  return {
    ...
    details: {
      value: `Word count: ${wordCount}, Avg words per sentence: ${avgWordsPerSentence.toFixed(
        1
      )}`,
      impact,
      context,
...
function analyzeImages($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  if (altTextPercentage < 100) {
    ...
    } out of ${totalImages} images missing alt text.`;
  } else {
    impact = "positive";
...
function analyzePageSpeed($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  return {
    ...
    category: "Page Speed",
    details: {
      value: `Scripts: ${scripts}, CSS: ${stylesheets}, Images: ${images}`,
      impact,
      context: `Found ${scripts} scripts, ${stylesheets} stylesheets, and ${images} images.`,
    },
  };
}

function analyzeSemanticStructure($: cheerio.CheerioAPI): DetailedSEOScore {
  ...
  return {
    ...
    details: {
      value: `Main: ${main}, Header: ${header}, Nav: ${nav}, Footer: ${footer}`,
      impact,
      context: "Analysis of semantic HTML structure",
    },
  };
}
...
async function enhancedAnalyzeSEO(url: string): Promise<EnhancedSEOReport> {
  ...
  try {
    ...
    const pageSpeedScore = analyzePageSpeed($);
    const semanticStructureScore = analyzeSemanticStructure($);
    ...
    if (error instanceof Error) {
      if (fetchMethod === "axios" && axios.isAxiosError(error)) {
        ...
        errorMessage = `${url} using advanced simulation. Please check if the website is currently accessible or verify that the URL is correct.`;
      }
    }
...
function generateEnhancedRecommendations(scores: {
  ...
  if (scores.titleScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.titleScore.details.context || "Title needs improvement",
      steps: [
      ...
      additionalContext:
        "Title tags are crucial for SEO and click-through rates.",
    });
  }
  ...
  if (scores.metaDescriptionScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.metaDescriptionScore.details.context ||
      ...
      additionalContext:
        "Meta descriptions impact click-through rates from search results.",
    });
  }
  ...
  if (scores.headingsScore.score < 100) {
    recommendations.push({
      ...
      description:
        scores.headingsScore.details.context ||
      ...
      additionalContext:
        "Proper heading structure improves both SEO and user experience, including for users with JavaScript disabled.",
    });
  }
  ...
  if (scores.imageOptimizationScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.imageOptimizationScore.details.context ||
      ...
      additionalContext:
        "Optimized images improve page load speed and accessibility.",
    });
  }
  ...
  if (scores.contentScore.score < 100) {
    recommendations.push({
      ...
      description:
        scores.contentScore.details.context || "Content needs improvement",
      steps: [
      ...
      additionalContext:
        "High-quality, comprehensive content is essential for SEO success, regardless of whether JavaScript is enabled.",
    });
  }
  ...
  if (scores.technicalScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.technicalScore.details.context ||
      ...
      additionalContext:
        "Technical SEO provides the foundation for overall SEO success.",
    });
  }

  // Add recommendation for noscript content if needed
  if (
    scores?.technicalScore?.details?.context?.includes(
      "No noscript tag found"
    ) ||
    scores?.technicalScore?.details?.context?.includes(
      "Empty noscript tag found"
    )
  ) {
    recommendations.push({
      ...
      additionalContext:
        "Noscript content improves accessibility for users without JavaScript and can be beneficial for SEO.",
    });
  }
  ...
  if (scores.mobileFriendlinessScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.mobileFriendlinessScore.details.context ||
      ...
      additionalContext:
        "Mobile-friendliness is crucial for both user experience and SEO.",
    });
  }
  ...
  if (scores.linkingStructureScore.score < 90) {
    recommendations.push({
      ...
      description:
        scores.linkingStructureScore.details.context ||
        "Linking structure needs improvement",
      steps: [
      ...
      additionalContext:
        "A good linking structure improves navigation and helps search engines understand your site.",
    });
  }

  // Add to generateEnhancedRecommendations function
  if (scores.semanticStructureScore?.score < 90) {
    recommendations.push({
      ...
      description:
        "Better semantic structure helps search engines understand your content",
      steps: [
        "Use <main> for primary content",
        "Add <header> and <footer> elements",
        "Implement proper <nav> elements",
        "Use <article> for independent content",
      ],
      additionalContext: "Semantic HTML improves accessibility and SEO",
    });
  }

  if (scores.pageSpeedScore?.score < 90) {
    recommendations.push({
      ...
      description:
        "Improve loading performance for better user experience and SEO",
      steps: [
      ...
      additionalContext: "Page speed is a crucial ranking factor",
    });
  }
